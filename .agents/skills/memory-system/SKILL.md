...
### Soft Cap on Inject (default 8192 chars)

`load_memory.py --inject` applies a **soft cap** so session context does not bloat:

1. Always include `MEMORY.md` index
2. Prefer core topics: conventions → preferences → ADR → ag-kit-defaults
3. Then newest remaining topics by mtime
4. Topics that do not fit → listed as deferred paths only (AI reads on demand)
5. Override: `--max-chars N` or `--no-cap`
6. Meta files (`README.md`, `CLEANUP-GUIDE.md`) are never injected as topics

### Pick Topics Per Chat

For a new chat that only needs some memories:
...
- `--topics` accepts names or 1-based indices from `--list-topics`
- Non-selected topics appear under "Not Selected" (paths only)
- Soft cap still applies to the selected set unless `--no-cap`
