...
paths:
  - "backend-nestjs/**"
description: >-
  Node.js/NestJS backend principles: async, security, architecture.
  Use when editing backend-nestjs or designing NestJS APIs.
...
## ⚡ Universal Node.js / NestJS Principles

1. **Queue Processors (BullMQ / RabbitMQ / Redis)**:
   - Separate queue processors into dedicated modules.
   - Always log job lifecycle events (pending, active, completed, failed) and handle failures gracefully.
2. **State & Metadata Strategy**:
   - Store complex transaction states or job metadata in structured JSON/schema columns to avoid over-normalizing transient data.
3. **External API Reliability**:
   - Wrap external third-party API calls in exponential backoff retry wrappers (`withRetry`).
