...
> 🔴 **MANDATORY:** Do NOT load every file in `.agents/agent/`. Load only agents listed in the session manifest.

**At session start:**

1. Check `.agents/.session/active-agents.json`.
2. If missing → run `python .agents/scripts/load_agents.py` to auto-detect stack and write the manifest.
3. Read **only** the agent `.md` files listed in `agents[]`. Skip the rest.

**Pivot rule (mid-session):**

When the user mentions a stack keyword that is NOT in the manifest's `stacks[]`
(see `pivot_keywords` in the manifest — e.g. "mobile", "rust", "flutter", "game"),
**stop and re-run** `python .agents/scripts/load_agents.py --add <stack>` (or `/reload <stack>`)
before routing to that domain's agent. Only after the manifest updates may you load the new agent.

**Rationale:** Loading all 16 agents + 47 top-level skills wastes context when working across many
projects with different tech stacks. The manifest keeps context lean and routing clean.
...
**Script inventory (canonical counts):**

| Tier | Location | Count |
| ---- | -------- | ----: |
| **Master** | `.agents/scripts/*.py` | **11** |
| **Skill-level** | `.agents/skills/*/scripts/*.py` | **16** |
| **Total** | | **27** |

**Skill-level audit scripts commonly used by `checklist.py` (subset of 16):**
...
> Master scripts (session boot, smoke, understand setup, …): `python .agents/scripts/<script>.py`
